...
import { prepareProject, sessionWorkspace, cleanupStaged, snapshotDir, diffSnapshot } from "@kawngraph/benchmark";
...
function makeSourceRepo(): { src: string; head: string | null } {
  const src = mkTmp("kawn-bench-src-");
...
test("A/B isolation: control vs treatment copies, one-time scan cost, workspaces, cleanup", async () => {
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(staged.base, ".kawn")), false, "control arm has no .kawn graph");
    assert.equal(fs.existsSync(path.join(staged.base, ".git")), false, ".git is never copied into a staged tree");
    assert.ok(
      fs.existsSync(path.join(staged.withBase, ".kawn", "graph.json")),
    ...
    // ---- the scanned graph is kept in memory for KawnGraph pack metrics -------
    ...
    assert.ok(wiCfg.mcpServers.kawn, "treatment arm registers the kawn MCP server");
    assert.ok(Array.isArray(wiCfg.mcpServers.kawn.args), "the server launch carries args");
...
test("prepareProject fails clearly when the source path does not exist", async () => {
  const missing = path.join(mkTmp("kawn-bench-missing-"), "nope");
...
test("snapshotDir + diffSnapshot detect added / modified / removed and ignore excluded dirs", () => {
  const dir = mkTmp("kawn-bench-snap-");
  try {
