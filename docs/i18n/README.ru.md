...
Установите и запустите KawnGraph **одной командой** — `npx` загрузит его, ничего
не нужно клонировать (Node ≥ 18):
...
**Или из исходников** (этот монорепозиторий, для контрибьюторов — [pnpm](https://pnpm.io)):
...
`setup` обнаруживает ваших кодовых агентов — **Claude Code**, **Codex**, **Cursor**,
**Copilot**, **Gemini CLI** и **Aider** (плюс `generic`-экспорт в Markdown/JSON
и опциональный **локальный LLM**) — и устанавливает **интеграцию только для чтения**,
ограниченную проектом (`.mcp.json`, `.cursor/mcp.json`, `.codex/config.toml`,
`.vscode/mcp.json`, `.gemini/settings.json` или файл контекста Aider), создавая резервные копии
всего, к чему прикасается, и проверяя каждый MCP-сервер живым рукопожатием. Полный
контракт: **[docs/AGENT_INTEGRATION.md](../AGENT_INTEGRATION.md)**.

**MCP-сервер** — это цикл JSON-RPC через stdio только для чтения **без MCP SDK** (написан вручную) с четырьмя инструментами:
...
уровень уверенности — механически выведенный там, где сканер может его прикрепить; каждый узел имеет
**стабильный, адресуемый по содержимому ID**, поэтому граф остаётся сравнимым между сканированиями.
Подробнее о модели:
...
<div align="center">
<img src="../assets/studio-universe.webp" alt="KawnGraph Studio — 3D-представление «Вселенная» только для чтения собственного графа этого репозитория: 1261 узел, сгруппированный по слоям (Code 815, Docs 430, Config 13, Data 3), с линиями связей, плюс фильтры по слоям/типам/рёбрам." width="860">
<br><sub>3D-представление <b>«Вселенная»</b> — собственный граф этого репозитория (1261 узел), только для чтения.</sub>
</div>

<div align="center">
<img src="../assets/studio-map.webp" alt="KawnGraph Studio — 2D-представление графа встроенного примера проекта: файлы, функции, маршруты, таблицы и документы как узлы с подписанными, подкреплёнными доказательствами рёбрами (imports, calls, defines, mentions, explains), плюс фильтры по слоям/типам/рёбрам." width="860">
<br><sub>2D-представление <b>графа</b> — встроенный пример проекта, с фильтрами по слоям / типам / рёбрам.</sub>
</div>
...
Universal Context Protocol, MCP-сервер только для чтения, настройка агентов одной командой
(Claude Code, Codex, Cursor, Copilot, Gemini, Aider, generic-экспорт, локальный LLM),
Studio и A/B-стенд для бенчмарков.
...
Создано и поддерживается **[Abdulrahman Alnashri](https://www.linkedin.com/in/abdulrahman-alnashri-ai/)**.
