...
| **Coding-agent integrations** | ~20+: Claude Code, Codex, Cursor, Copilot CLI/Chat, Aider, Gemini CLI, OpenCode, Kilo/Roo/Cline, Amazon Q, Kiro, Goose, Antigravity, … | **3**: Claude Code, Codex, Cursor — reversible, project-scoped MCP config only. See `packages/agents`, `tests/agents.test.ts`. | **Graphify integrates with many more clients.** KawnGraph's three adapters are contract-tested and reversible. |
